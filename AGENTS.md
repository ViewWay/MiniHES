...
- **数据库:** PostgreSQL + Redis + MongoDB（PG 存结构化数据，MongoDB 存 DLMS 采集原始大文档，详见 `docs/design/mongodb-integration.md`）
- **包管理:** uv (后端), pnpm (前端)
- **部署:** Docker Compose（PostgreSQL + Redis + MongoDB 一键启动）+ systemd 双支持
...
# 注：InfluxDB 已弃用，MongoDB 替代其作为大文档存储
```
