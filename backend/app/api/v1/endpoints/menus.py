...
from app.services.menu_service import list_menus, check_name_exists, check_path_exists
...
async def check_name_exists_endpoint(
    ...
    name: str = Query(...), id: int | None = Query(default=None),
):
...
async def check_path_exists_endpoint(
    db: DbSession = ..., _user: CurrentUser = ...,
    path: str = Query(...), id: int | None = Query(default=None),
):
