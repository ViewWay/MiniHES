...
async def list_meter_types():
    async with async_session() as session:
        result = await session.execute(select(MeterType).order_by(MeterType.id))
        items = [
...
async def list_wire_types():
    async with async_session() as session:
        result = await session.execute(select(WireType).order_by(WireType.id))
        items = [
...
async def list_meter_points(protocol: str = Query(default=None)):
    async with async_session() as session:
...
async def create_meter_point(body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
        point = MeterPoint(
...
async def update_meter_point(point_id: int, body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
        result = await session.execute(select(MeterPoint).where(MeterPoint.id == point_id))
        point = result.scalar_one_or_none()
        if not point:
            return fail(code=10001, message="采集点不存在", status=404)
...
async def delete_meter_point(point_id: int, _=Depends(get_current_user)):
    async with async_session() as session:
        result = await session.execute(select(MeterPoint).where(MeterPoint.id == point_id))
        point = result.scalar_one_or_none()
        if not point:
