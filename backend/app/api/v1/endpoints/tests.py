...
@router.post("")
async def create_test(body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
        t = TestTask(**body)
        session.add(t)
...
async def get_test(test_id: int):
    async with async_session() as session:
        result = await session.execute(select(TestTask).where(TestTask.id == test_id))
...
async def create_defect(test_id: int, body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
...
async def create_test_report(test_id: int, body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
...
async def update_test_report(test_id: int, body: dict, _=Depends(get_current_user)):
    async with async_session() as session:
        result = await session.execute(
            select(TestReport).where(TestReport.test_id == test_id)
        )
...
async def export_test_report(test_id: int, _=Depends(get_current_user)):
    return success({"message": "报告导出功能待实现"})


@router.post("/{test_id}/report/distribute")
async def distribute_report(test_id: int, body: dict, _=Depends(get_current_user)):
